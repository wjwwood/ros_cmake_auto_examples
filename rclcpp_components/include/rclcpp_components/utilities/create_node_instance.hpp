...
#ifndef RCLCPP_COMPONENTS__UTILITIES__CREATE_NODE_INSTANCE_HPP_
#define RCLCPP_COMPONENTS__UTILITIES__CREATE_NODE_INSTANCE_HPP_
  ...
  for (const auto & node_plugin_pair : node_plugins) {
    ...
    for (auto class_name : class_names) {
      if (node_class_name == class_name) {
        ...
        return std::unique_ptr<NodeInstance>(new NodeInstance({loader, node}));
      }
    }
...
#endif  // RCLCPP_COMPONENTS__UTILITIES__CREATE_NODE_INSTANCE_HPP_
